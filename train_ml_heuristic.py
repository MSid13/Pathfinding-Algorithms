...
while len(residual_targets) < NUM_SAMPLES:
    ...
    if cost is not None:
        ...
        if residual < 4.0: #increase to increase the difficulty of testing
            continue
        sg_features = [
...
x1 = layers.Conv2D(16, (3,3), activation='relu', padding='same')(grid_input)
x1 = layers.MaxPooling2D((2,2))(x1)
x1 = layers.Conv2D(32, (3,3), activation='relu', padding='same')(x1)
x1 = layers.Flatten()(x1)

sg_input = Input(shape=(4,), name='start_goal')
x2 = layers.Dense(32, activation='relu')(sg_input)
...
model.export("ml_heuristic_savedmodel")
print("Model exported as TensorFlow SavedModel (ml_heuristic_savedmodel/)")

# converting to tensorflow:
# docker run --rm -v "${PWD}:/workspace" tfjs_converter --input_format=tf_saved_model --output_format=tfjs_graph_model ml_heuristic_savedmodel web_model
